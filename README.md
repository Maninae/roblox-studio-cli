...
`luau` takes its source three ways: as an argument, from a file with `--file script.luau`, or from stdin with `-`. Both of the ways that read something else's bytes are capped at 8 MB, `--file` and `-` alike, and a `--file` must be an ordinary file.
